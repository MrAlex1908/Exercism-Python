...
def round_scores(student_scores):
    ...
    while student_scores: 
        rounded_score = student_scores.pop()
        rounded_scores.append(round(rounded_score))
...
def letter_grades(highest):
    ...
    for score in range(41, highest, step):
        lowest_ranges.append(score)
    return lowest_ranges    
...
def student_ranking(student_scores, student_names):
    ...
    for index, student_name in enumerate(student_names):
        rank = index + 1
        line = f"{rank}. {student_name}: {student_scores[index]}"
        rank_students_list.append(line)
...
def perfect_score(student_info):
    ...
    """
    for index, students_info in enumerate(student_info):
        if students_info[1] == 100:
