...
def value_of_card(card):
    ...
    """
    high_value_cards = ['J', 'Q', 'K']
    ace = 'A'
    low_value_cards = ['2', '3', '4', '5', '6', '7', '8', '9', '10']

    if card in high_value_cards:
        return 10
    elif card == ace:
        return 1
    elif card in low_value_cards:
        return int(card)
    else:
        raise ValueError('Invalid card value. Please provide a valid card (2-10, J, Q, K, A).')
...
def value_of_ace(card_one, card_two):
    ...
    if card_one == 'A':
        ...
        total_value += value_of_card(card_one)
    if card_two == 'A' and card_one != 'A':
        total_value += 11
    else:
        total_value += value_of_card(card_two)

    return 1 if total_value > 10 else 11
