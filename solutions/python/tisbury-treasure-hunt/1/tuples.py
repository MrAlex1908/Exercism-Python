...
def clean_up(combined_record_group):
    ...
    for record in combined_record_group:
        cleaned_record = []
        for item_1 in record:
            is_duplicate = False
            for item_2 in record:
                if isinstance(item_1, str) and len(item_1) == 2:
                    if convert_coordinate(item_1) == item_2:
                        is_duplicate = True
            if is_duplicate is False:
                cleaned_record.append(item_1)
        cleaned_records.append(tuple(cleaned_record))
    
    result = ""
    for string in cleaned_records:
        result += f"{string}\n"
    return result